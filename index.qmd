...
How can someone compare two search functions against each other? Any benchmark would require not only a (synthetic or real) document corpus and set of queries, but also a ground truth document for each query. Assuming the corpus and query set already exists, a benchmark can be created by employing some extremely time-consuming but high-fidelity search approach; it's not hard to think of high-quality search tactics that are impractical because of their slow runtime. These approaches can be applied to benchmark creation.
